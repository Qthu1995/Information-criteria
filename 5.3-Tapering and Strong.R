...
simulation.data <- function(nsims=500, nsamples=seq(10, 150, 5), alpha=0, 
                            beta=0, nX=10, meanX=0, inX=rep(TRUE,nX), XCov=diag(nX)){
  ...
  for(i in 1:n.sample.sizes){
    ...
    for(j in 1:nsims){
      ...
      reg.eqn <- as.formula(paste("y",paste("",x.in,sep="",collapse="+"),sep="~"))
...
diag(XCov) <- 1
strong2 <- simulation.data(alpha=3, beta=3, nX=10, inX=c(rep(TRUE,5),rep(FALSE,5)), XCov=XCov)

XCov <- matrix(rep(0.5, 21*21), ncol=21)
diag(XCov) <- 1
tapering2 <- simulation.data(alpha=3, beta=exp(seq(log(3),log(0.005),length=21)), 
                             nX=21, inX=c(rep(c(TRUE,rep(FALSE,4)),4),TRUE), XCov=XCov)
